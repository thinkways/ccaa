...
function setting(){
	...
	sed -ir "s@#rpc-certificate=.*@rpc-certificate=/etc/ccaa/cert.pem@g" /etc/ccaa/aria2.conf
	sed -ir "s@#rpc-private-key=.*@rpc-private-key=/etc/ccaa/key.pem@g" /etc/ccaa/aria2.conf
